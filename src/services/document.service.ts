...
class DocumentService {
  ...
  async getAllDocumentsByOrganization(organizationId: string): Promise<any[]> {
    try {
      const [invoices, purchaseOrders, grns, paymentAdvices] =
        await Promise.all([
          InvoiceService.getInvoicesByOrganization(organizationId),
          PurchaseOrderService.getPurchaseOrdersByOrganization(organizationId),
          GRNService.getGRNsByOrganization(organizationId),
          PaymentAdviceService.getPaymentAdvicesByOrganization(organizationId),
        ]);
      ...
      const transformedPaymentAdvices = paymentAdvices.map((pa) => ({
        ...pa,
    ...
    try {
      ...
      const documents = await Promise.all(
        documentIds.map(async (id) => {
          try {
            ...
            logger.info(`Found ${docType} document with ID: ${id}`);
            return doc;
          } catch (error) {
      ...
      const validDocuments = documents.filter((doc) => {
        ...
        if (!doc.extracted_data) {
          logger.warn(`Document ${doc._id} has no extracted data`);
          return false;
        }
        if (doc.extracted_data.error) {
          logger.warn(`Document ${doc._id} has extraction error`);
          return false;
        }
        return true;
      });
      ...
      logger.info(
        `Found ${validDocuments.length} valid documents out of ${documents.length}`
      );
      ...
      for (const doc of validDocuments) {
        ...
        if (!docConfig) {
          logger.warn(`No configuration found for document type: ${docType}`);
          continue;
        }

        logger.info(`Processing ${docType} document: ${doc._id}`);
      ...
      logger.info(
        `Successfully exported ${validDocuments.length} documents to Excel for organization ${orgCode}`
      );
      return excelBuffer;
    } catch (error) {
